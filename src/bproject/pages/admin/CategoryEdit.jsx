...
import { categoryUpdate, getCategory } from "./js/adminMain";

export default function CategoryEdit({title}) {

    // 경로
    const location = useLocation();
    const reData = location.state;

    const {data: ca} = getCategory(reData?.ca_id); 
                    ...
                    <table>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <span className="frm_info">자동으로 보여지는 분류코드를 사용하시길 권해드리지만 직접 입력한 값으로도 사용할 수 있습니다.<br />분류코드는 나중에 수정이 되지 않으므로 신중하게 결정하여 사용하십시오.<br /><br />분류코드는 2자리씩 10자리를 사용하여 5단계를 표현할 수 있습니다.<br />0~z까지 입력이 가능하며 한 분류당 최대 1296가지를 표현할 수 있습니다.<br />그러므로 총 3656158440062976가지의 분류를 사용할 수 있습니다.</span> <input type="number" name="ca_id" value={ca?.ca_id} id="ca_id"  className="required frm_input" size="2" maxLength={2} readOnly />
                                </td>
                            </tr>
                            <tr>
                                <th scope="row"><label htmlFor="a_name">분류명</label></th>
                                <td><input type="text" name="ca_name"  id="ca_name" size="38"  className="required frm_input" defaultValue={ca?.ca_name} /></td>
                            </tr>
                            <tr>
                                <th scope="row"><label htmlFor="a_order">출력순서</label></th>
                                <td>
                                    <span className="frm_info">숫자가 작을 수록 상위에 출력됩니다. 음수 입력도 가능하며 입력 가능 범위는 -2147483648 부터 2147483647 까지입니다.<br /><b>입력하지 않으면 자동으로 출력됩니다.</b></span> <input type="number" name="ca_order"  id="ca_order" className="frm_input" size="12"  defaultValue={ca?.ca_order} />
                                </td>
                            </tr>
